...
## 第五步（可选）— Chrome DevTools MCP（付费墙兜底）

当 scansci-pdf 所有通道失败时，Chrome DevTools MCP 可直接通过你已登录的 Chrome 浏览器下载。**不需要配置 WebVPN URL、不需要导出 cookies**，直接复用你日常浏览器的登录状态。

**适用场景：** Wiley 非 OA、未被 Sci-Hub 收录的新论文、知网直接搜索。

详见 `references/chrome-devtools.md`。
...
| 完整中文 + 付费墙兜底 | + 第五步 | + 知网机构搜索、Wiley 等浏览器直下 |
...
> **给 AI 的提示：** 不要通过 shell 工具运行 `setup.ps1` 或任何 scansci-pdf 登录工具 —— 它们需要交互式终端和可见浏览器。告诉用户确切命令让其自己运行。Chrome DevTools MCP 工具（`navigate_page`、`evaluate_script` 等）可由 AI 直接调用，无需用户干预。
